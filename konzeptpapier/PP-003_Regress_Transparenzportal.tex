...
Die beiden Prüfverfahren widersprechen sich fundamental: wissenschaftlich, ökonomisch und rechtlich. Diesen Widerspruch muss man verstehen, bevor man die Zahlen interpretiert.
...
\subsubsection{Drei Scanner, keine Qualitätsprüfung}

Das Wirtschaftlichkeitsprüfsystem betreibt drei Screening-Mechanismen. Keiner davon prüft medizinische Versorgungsqualität (ausführliche Analyse in ST-001, Abschn.~2.8):
...
\begin{itemize}[nosep]
  ...
  \item \textbf{V2a (Einzelfallprüfung --- Wirtschaftlichkeit):} Prüft Wirtschaftlichkeit einzelner Verordnungen. Medizinische Begründung und Kostendifferenz möglich. Prüft nicht, ob die Therapieentscheidung medizinisch richtig war.
  \item \textbf{V2b (Einzelfallprüfung --- Formfehler):} Scannt formale Korrektheit (Unterschrift, ICD$\leftrightarrow$PZN, Dosierung). Medizinische Korrektheit irrelevant.
...
\textbf{Niemand prüft}, ob der Patient ausreichend versorgt wurde (\S\,12 SGB\,V), ob eine Unterlassung stattfand, ob die Therapieentscheidung leitliniengemäß war, oder ob der Arzt aus Angst \textit{weniger} verordnet hat als medizinisch geboten. Das System bestraft Formfehler --- nicht Überversorgung im medizinischen Sinne. Ein Arzt, der formal korrekt dokumentiert aber medizinisch schlecht behandelt, ist unsichtbar. Ein Arzt, der exzellente Medizin macht aber einen Formfehler produziert, ist maximal exponiert. Ein Arzt, der aus Angst gar nicht erst verordnet, wird nie geprüft --- aber der Patient trägt die Folgen (Unterversorgung, Überweisung, Hospitalisierung). Der Arzt selbst riskiert im Einzelfall eine Behandlungsfehler-Klage (\S\,280 BGB) --- aber die hat kein Prüfverfahren als Auslöser, sondern den Patienten.

\subsubsection{Der doppelte Zertifikatsfehler: Was das System tatsächlich misst}

Beide Prüfverfahren messen nicht, was \S\,12 SGB\,V als Schutzziel benennt — bedarfsgerechte, zweckmäßige Versorgung. Sie messen Proxys (Ersatzgrößen, die stellvertretend für das eigentliche Ziel gemessen werden):
...
\midrule
Auffälligkeitsprüfung & wirtschaftliche Versorgung & Kostenabweichung vom Fachgruppendurchschnitt \\
Einzelfallprüfung & medizinische Unwirtschaftlichkeit im Einzelfall & formale und dokumentarische Angreifbarkeit \\
\midrule
\textbf{Kein Verfahren} & \textbf{\S\,12 SGB\,V} & \textbf{wird systematisch nicht gemessen} \\
\bottomrule
\end{tabularx}
}

Die Einzelfallprüfung beansprucht, medizinische Unwirtschaftlichkeit zu prüfen, misst aber faktisch formale Angreifbarkeit. Formfehler sind: billig zu prüfen, eindeutig, angstwirksam, nicht medizinisch streitbar. Das erzeugt eine inverse Selektionslogik: Das System selektiert gegen Dokumentationsschwäche, nicht gegen Versorgungsdefizite. In der Sozialwissenschaft ist dieses Phänomen als \textit{Goodhart's Law} bekannt: Wenn eine Kennzahl zum Steuerungsziel wird, hört sie auf, eine gute Kennzahl zu sein (Goodhart 1975; Campbell 1979). Der Zertifikatsfehler ist keine Anomalie, sondern eine bekannte systemische Pathologie von Proxy-Messungen. \textbf{Medizinische Angemessenheit wird nur außerhalb des Regresssystems zertifiziert} — in Schlichtungsverfahren und Arzthaftungsprozessen. Das Transparenzportal (PP-003) löst diesen Zertifikatsfehler nicht, sondern macht ihn sichtbar und messbar. Die vollständige Analyse in ST-001, Abschn.~2.5. Einschränkung: Der Zertifikatsfehler trifft in voller Schärfe V2b (Formfehler). V1 berücksichtigt medizinische Besonderheiten im Schutzstufenmodell. V2a lässt medizinische Verteidigung zu.
...
\textbf{Gesundheitswesen} & \textbf{Keine Heilung.} Jeder Kodierfehler wird finanziell sanktioniert (\S\,275c SGB\,V). \\
\bottomrule
\end{tabularx}
}
...
\subsubsection{BSG B\,6\,KA\,9/24\,R: 491.000\,EUR für einen Stempel}

Am 27. August 2025 bestätigte das Bundessozialgericht einen Regress von 491.000\,EUR über 14 Quartale — weil ein Arzt seine Verordnungen mit Namensstempel statt persönlicher Unterschrift gezeichnet hatte. Die medizinische Korrektheit der Verordnungen war unbestritten. Das BSG urteilte: Die persönliche Unterschrift sei kein bloßer Formalakt, sondern diene dem ``Schutz des Lebens''. Eine nachträgliche Heilung des Formfehlers wurde nicht zugelassen.
...
UWG / Wettbewerbsrecht & \textbf{Sehr gering} & Gemeinnütziger Verein = kein geschäftliches Handeln \\
...
Die KV Rheinland-Pfalz veröffentlicht seit 2025 ein öffentliches Regress-Ranking mit Kassen-Rankings nach § 106b/106d SGB V. Wenn eine Körperschaft des öffentlichen Rechts solche Daten publiziert, ist es einem gemeinnützigen Verein erst recht nicht verwehrt.
...
\textbf{Kosten:} Porto (~1 \,EUR/Arzt) + Druckkosten → vernachlässigbar


\begin{itemize}[nosep]
  \item Integration des \textbf{elektronischen Heilberufsausweises (eHBA)}
  \item Seit 01/2021 Pflicht für alle Vertragsärzte
  \item Höchste Verifikationsstufe
  \item Technisch aufwändiger (Chipkarten-Lesegerät, Middleware)
\end{itemize}


\begin{itemize}[nosep]
  \item Viele Ärzte haben Fax, aber kein Chipkartenlesegerät
  \item Fax an Praxis-Faxnummer mit Aktivierungscode
  \item Praxis-Faxnummern sind öffentlich
\end{itemize}
...
\begin{itemize}[nosep]
  \item BSG 27.08.2025, B 6 KA 9/24 R — Formalregress 491.000\,EUR (Stempel statt Unterschrift)
...
\begin{itemize}[nosep]
  ...
  \item Moosazadeh et al. 2023 (Int J Qual Health Care): Meta-Analyse Defensivmedizin (75,8\,\% Prävalenz)
  \item Stiftung Datenschutz: Praxisleitfaden Anonymisierung
\end{itemize}
...
\begin{itemize}[nosep]
  \item \textbf{ST-001 — Systemtheoretische Aufarbeitung der Regressangst} (Begleitstudie, v0.9, 66 Seiten): historische Container-Genealogie, rechtsdogmatische Drei-Code-Kollision (\S\,12 SGB\,V), Spieltheorie, Modellarzt-Hochrechnung (0,9--1,7\,Mrd.\,EUR/Jahr Folgekosten), Niskanen-Bestätigung der Prüfungseinrichtungen, verfassungsrechtliche Doppelbeschwerde-Strategie, normativer Schadensbegriff (BSG B\,6\,KA\,5/09\,R), Lesart~C (System als Formfehler-Optimierer), Prüfkatalog-Asymmetrie, Gegenpositions-Abschnitt (5 stärkste Einwände), Anhang~C (9 IFG-Anfragen als offene Forschungsfragen).
